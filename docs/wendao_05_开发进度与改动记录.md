...
## v40.12：第一章扎根 Step 1+2 (2026-04-25)

**总体方向**：v40.11 后从"打磨叶子"转向"扎根"。意识到第一章框架两个空洞——
世界规则没交代、苏云晚个人 stake 没建立。本版基于 sui-yu-zhen-bible.md v0.1
分两步落地。

### Step 1：morning 重写（爹的对白 + 苏云晚 B1 内心 + 选择支差异化）

**问题**：原 morning 只有温度（爹"回来吃饭+桂皮"），没有世界规则、没有苏云晚
内心 stake。爹的话无法承担"修仙世界轮廓"，玩家不知道测出灵根意味着什么。

**改动**（`data/chapter1.json`）：

- 新插入 `m_04b`：苏云晚 B1 内心独白
  > "她想测出来。不是为修仙，是想看看镇外是什么样。也不想测出来——手心里这点温度，舍不得放下。"
- 爹的对白拆三拍 `m_07 / m_08 / m_08p / m_08w`：
  > "云晚。" → "测出来的……早晚要跟人走的。" → [stops, narration "话像想了一路，到嘴边收了一半回去"] → "不管测出什么，记得回来吃饭。顺路去集市看看有没有桂皮，家里的快用完了。"
- `m_08b` 选择支差异化（flag 名沿用，含义改）：
  - 暖支：「她点了点头。出门时她回头看了一眼，爹正望着院里那棵桂花树」→ `father_morning_warm`
  - 静支：「她点了点头，没回头」→ `father_morning_silent`
- `m_09` 兜底：「前半句没接的那半句，她没敢多想」
- 下游同步——`letter/lt_06c_warm` 和 `boss_awakening/ba_cb_father` 两处闪回画面
  从"在门口多停了一下"换成"她回头那一眼——爹望着桂花树"

### Step 2：走向广场途中 NPC 浮字 + 设定层八品门槛

**问题**：bible §10 要求"走向广场途中 NPC 闲聊承担频率感+历史统计+苏云晚私人 stake"。
现有 5 条 BUBBLE_NODES 不够覆盖这三件事。

**设定层修订**（先于文案）：

- bible §一 加入"九品分级、数字越小越顶级、八品=最低门槛"
- bible §一 新增"碎玉镇视野的局限"段——太平宗门槛=八品=镇民语境天花板，
  三品/一二品/更大宗门镇民"想不到那么远"。**反向暗示更大世界**
- bible §四"苏云晚个人 stake"重写——砍掉"五六品就好"等具体数字（她不知道自己
  能力，不可能内行思考），改成"和镇民同形的小指望（测出来 + 还能回）"。
  新增"区别在态度"段——她**两头都有牵挂**，外人看像不在乎，里头指望是真的
- `docs/wendao_06`：太平宗门槛 五品以上录 → 八品以上录
- `chapter1.json` 公告栏文案：五品及以上者 → 八品及以上者
- `chapter1.json` 镇民闲聊：一个五品以上的都没有 → 一个八品以上的都没有
- `chapter1.json` 茶馆掌柜"金灵根五品被太平宗当场带走"**保留**——五品远高于
  八品门槛，被秒抢逻辑反而更顺

**文案改动**（`scripts/TownScene.gd` 的 `BUBBLE_NODES`）—— 新增 3 条：

- (11, 12) 苏云晚 inner："若真测出来，学几年还能回来。"
- (13, 12) 镇民焦虑+频率："三年了，再不测出来，又得等三年。"
- (24, 12) 历史统计双面："十年带走十一个。回来的不到一半。"
- 数组按地理位置西→东排序，便于以后调位

**机制改动**（`scripts/TownScene.gd` 的 `_check_bubble_triggers`）：
单次触发改成"离开范围后允许再触发"。两遍扫描——先重置已离开范围的，再触发未触发的。
代价：玩家来回走会反复看到同一条；好处：错过找不回来的 UX 问题彻底解决
（_bubble_timer 3 秒间隔保证不刷屏）。

### 文件变更清单

```
修改：
  design/narrative/sui-yu-zhen-bible.md  (§一 + §四 修订)
  data/chapter1.json                     (morning 重写 + 闪回画面同步 + 公告栏/闲聊"五品→八品")
  docs/wendao_06_有意义互动补全设计.md   (招募门槛 五品→八品)
  scripts/TownScene.gd                   (+3 条 BUBBLE_NODES + bubble 可重触发机制)
```

### 仍需测试

- morning 三拍节奏（"测出来的……早晚要跟人走的" 省略号停顿是否被吃掉）
- 暖支选择支后接 m_09"没敢多想"是否自相矛盾
